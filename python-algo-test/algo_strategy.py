...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def build_defences_1(self, game_state):
        ...
        support_locations.extend([[14, 6]])
        game_state.attempt_spawn(SUPPORT, support_locations)
        game_state.attempt_upgrade(support_locations)

        # Place walls on corners
        wall_locations.extend([[0, 13], [1, 13], [26, 13], [27, 13]])
        ...
        wall_locations.extend([[12, 8], [15, 8]])
        game_state.attempt_spawn(WALL, wall_locations)
        ...
        support_locations.extend([[12, 6], [13, 6], [14, 6], [15, 6], [13, 5], [13, 6]])
        game_state.attempt_upgrade(support_locations)
        game_state.attempt_spawn(SUPPORT, support_locations)

        # Build More Walls and Turrets
        wall_locations.extend([[0, 13], [1, 13], [2, 13], [3, 13],
        ...
        turret_locations.extend([[2, 12], [25, 12], [6, 10], [21, 10], [9, 8], [18, 8]])
        game_state.attempt_spawn(TURRET, turret_locations)
        game_state.attempt_upgrade(turret_locations)
        game_state.attempt_spawn(WALL, wall_locations)
        game_state.attempt_upgrade(wall_locations)
